...
export default function CitasPage() {
  ...
  if (sucRes.status === 'fulfilled') {
    ...
    console.warn('No se pudieron cargar las sucursales:', sucRes.reason);
  }

  if (vehRes.status === 'fulfilled') {
    setVehiculos(vehRes.value.data);
  } else {
    console.warn('No se pudieron cargar los vehículos:', vehRes.reason);
  }

  if (servRes.status === 'fulfilled') {
    setServicios(servRes.value.data);
  } else {
    console.warn('No se pudieron cargar los servicios:', servRes.reason);
  }
  ...
  if (citasRes.status === 'fulfilled') {
    ...
    setCitas([]);
    console.warn('No se pudieron cargar las citas:', citasRes.reason);
  }
  ...
  if (!clienteId) {
    showActionError('No se pudo identificar tu perfil de cliente. Cierra sesión e inicia sesión nuevamente.');
    return;
  }
...
/* PASO 5: variables estáticas para requisitos */
...
const requisitosPendientes = [
  ...
  noHaySucursales ? 'No hay sucursales disponibles. Un administrador debe registrar al menos una sucursal.' : null,
  noHayServiciosActivos ? 'No hay servicios activos. Un administrador debe activar al menos un servicio del catálogo.' : null,
    ...
    <div className="rounded-lg border border-red-200 bg-red-50 p-4 text-red-700">
      ...
      {citaACancelar && (
        <div className="mt-2 space-y-1 text-sm">
          ...
          <p>
            <span className="font-medium">Hora:</span> {citaACancelar.hora}
          </p>
