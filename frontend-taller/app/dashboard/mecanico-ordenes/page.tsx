...
import { inventarioApi, ordenApi, servicioApi } from '@/lib/api';
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const activas = ordenesRes.data.filter(
        (orden: OrdenTrabajo) => !['COMPLETADA', 'CANCELADA'].includes(orden.estado)
      );
      ...
      setSelectedOrden((prev) => {
        if (!prev) return null;
        return activas.find((orden: OrdenTrabajo) => orden.id === prev.id) || null;
      });
    } catch (err: any) {
      if (requestId !== requestIdRef.current) return;
              ...
              <div className="mb-4 grid grid-cols-2 gap-4 text-sm text-gray-600">
                ...
                {selectedOrden.fechaFinalizacionEstimada && (
                  <div>
                    ...
                    {new Date(selectedOrden.fechaFinalizacionEstimada).toLocaleDateString('es-SV')}
                  </div>
                )}
              </div>
              ...
              {selectedOrden.presupuestoTotal && (
                <div className="mb-4 rounded border border-yellow-200 bg-yellow-50 p-3 text-sm">
                  <strong className="text-yellow-800">
                    Presupuesto enviado: ${Number(selectedOrden.presupuestoTotal).toFixed(2)}
                  </strong>
              ...
              {showPresupuestoForm && (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        Agrega servicios o repuestos si el diagnóstico requiere trabajo adicional.
                      </p>
                    </div>
                  ...
                  <div className="rounded-lg bg-blue-50 p-3 text-blue-800">
                    ...
                    <p className="text-2xl font-bold">${totalPresupuestoCalculado.toFixed(2)}</p>
